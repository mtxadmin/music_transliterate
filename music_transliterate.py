...
class FileTransliterator:
    ...
    def _create_transliteration_tables(self) -> Dict[str, Dict[str, str]]:
        ...
        # Общая таблица для европейских языков с диакритикой (расширенная)
        european_table = {
            ...
            'ä': 'a', 'å': 'a', 'æ': 'ae', 'ç': 'c', 'è': 'e',  # è есть здесь!
            'é': 'e', 'ê': 'e', 'ë': 'e', 'ì': 'i', 'í': 'i',
            'î': 'i', 'ï': 'i', 'ð': 'd', 'ñ': 'n', 'ò': 'o',
            'ó': 'o', 'ô': 'o', 'õ': 'o', 'ö': 'o', 'ø': 'o',
            'ù': 'u', 'ú': 'u', 'û': 'u', 'ü': 'u', 'ý': 'y',
    ...
    def transliterate_text(self, text: str, language: Optional[str] = None, check_ascii: bool = True) -> Optional[str]:
        ...
        for char in text:
            ...
            elif self._is_diacritic(char):
                # Для диакритических символов в других языках также применяем нормализацию
                normalized = unicodedata.normalize('NFD', char)
                base_char = ''.join(c for c in normalized if not unicodedata.combining(c))
                result_chars.append(base_char if base_char else char)
            else:
    ...
    def _is_diacritic(self, char: str) -> bool:
        """Проверяет, является ли символ диакритическим или содержит диакритику"""
        # Проверяем, является ли символ комбинированным диакритическим знаком
        if unicodedata.combining(char):
            return True
        
        # Проверяем, содержит ли символ диакритические знаки после декомпозиции
        try:
            # Пробуем декомпозицию
            decomposed = unicodedata.normalize('NFD', char)
            # Если после декомпозиции есть комбинированные диакритические знаки
            if any(unicodedata.combining(c) for c in decomposed):
                return True
        except:
            pass
        
        # Проверяем по диапазонам Unicode для диакритических символов
        code = ord(char)
        
        # Основные диапазоны Unicode для букв с диакритикой:
        ...
        elif 0x2C60 <= code <= 0x2C7F:
            return True
        # Latin Extended-D (A720-A7FF)
        elif 0xA720 <= code <= 0xA7FF:
            return True
        # Latin Extended-E (AB30-AB6F)
        elif 0xAB30 <= code <= 0xAB6F:
            return True
